...
-- Schema reference snapshot: AI virtual try-on jobs
-- Supabase project: oazipcrbutizdncctkcg
-- Branch: feature/product-admin
-- IMPORTANT: Reference only. Storage bucket policies are maintained separately.
...
create table if not exists public.try_on_jobs (
  ...
  category text not null default 'top' check (category = any (array['top','bottom','dress','outerwear','full_body'])),
...
create index if not exists try_on_jobs_status_created_at_idx
  on public.try_on_jobs(status, created_at);
...
-- AI provider secrets belong in server-side environment variables.
-- Result/image storage access is controlled separately from this table.