-- Product Vision attributes generated from catalog product images.
-- The backend writes these records with the Supabase service-role key.

create table if not exists public.product_vision_attributes (
  product_id text primary key references public.products(id) on delete cascade,
  garment_type text not null default 'unknown'
    check (garment_type in ('top', 'bottom', 'dress', 'outerwear', 'accessory', 'unknown')),
...
on public.product_vision_attributes
for select
to public
using (true);
