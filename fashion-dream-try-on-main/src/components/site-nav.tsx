...
export function SiteNav() {
  ...
  useEffect(() => {
    let mounted = true;
    const sync = async () => { const user = await getCustomerUser(); if (mounted) setCustomerEmail(user?.email ?? null); };
    void sync();
    ...
    return () => { mounted = false; window.removeEventListener("upthink:auth:login", onLogin); window.removeEventListener("upthink:auth:logout", onLogout); };
  }, []);
  ...
  const submitSearch = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const value = query.trim(); if (!value) return; window.location.href = `/shop?search=${encodeURIComponent(value)}`; closeMenus(); };
  ...
  const toggleSearch = () => { setSearchOpen((value) => !value); setOpen(false); setShopOpen(false); setNotificationOpen(false); };
  ...
  return <nav className="fashion-nav" aria-label={t("Điều hướng chính", "Main navigation")}>
    ...
    <div className="fashion-nav__inner">
      <Link to="/" className="fashion-brand" aria-label="WEARO home" onClick={closeMenus}><span className="fashion-brand__mark" aria-hidden="true">W</span><span className="fashion-brand__copy"><span className="fashion-brand__name">WEARO<span>.</span></span><span className="fashion-brand__meta">MODERN EVERYDAY / 2026</span></span></Link>
      <div className="fashion-nav__links" aria-label={t("Điều hướng trang chính", "Primary navigation")}>
        <div className={`fashion-shop-nav ${shopOpen ? "is-open" : ""}`} onMouseEnter={() => setShopOpen(true)} onMouseLeave={() => setShopOpen(false)}>
          <Link to="/shop" className="fashion-nav-link fashion-shop-trigger" aria-expanded={shopOpen} aria-haspopup="true" onClick={() => { setSearchOpen(false); setOpen(false); setNotificationOpen(false); }} style={TOPBAR_TEXT_STYLE} activeProps={{ className: "fashion-nav-link fashion-shop-trigger is-active" }}><span>{t("Cửa hàng", "Shop")}</span><ChevronDown size={15} strokeWidth={2} aria-hidden="true" /></Link>
          {shopOpen && <div className="fashion-shop-mega" role="dialog" aria-label={t("Danh mục sản phẩm", "Shop categories")}><div className="fashion-shop-mega__intro"><span className="fashion-menu-kicker">WEARO / COLLECTIONS</span><h2>{t("Mua theo phong cách", "Shop your style")}</h2><Link to="/shop" onClick={closeMenus} className="fashion-shop-all">{t("Xem tất cả sản phẩm", "View all products")} <ChevronRight size={15} /></Link></div><div className="fashion-shop-discovery-grid">{DISCOVERY_LINKS.map((link) => <Link key={link.label} to={link.to} onClick={closeMenus} className="fashion-shop-all"><span>{t(link.vi, link.label)}</span><ChevronRight size={15} /></Link>)}</div><div className="fashion-shop-category-grid">{CATEGORIES.map((category) => <Link key={category.slug} to="/shop" search={{ category: category.slug }} onClick={closeMenus} className="fashion-shop-category"><span className="fashion-shop-category__image"><img src={category.image} alt="" loading="lazy" /></span><span className="fashion-shop-category__meta"><strong>{category.name}</strong><span>{t("Khám phá danh mục", "Explore category")}</span></span><ChevronRight size={15} aria-hidden="true" /></Link>)}</div></div>}
        </div>
