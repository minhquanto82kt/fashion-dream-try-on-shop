...
function ProductPage() {
  ...
  const gallery = Array.from(new Set(product.gallery.filter(Boolean)));
  ...
  return <div className="min-h-screen"><SiteNav /><main className="mx-auto max-w-7xl px-6 pb-24 pt-28 sm:px-12 lg:px-20">
    ...
    <div className="grid min-w-0 gap-10 lg:grid-cols-[minmax(0,1.08fr)_minmax(360px,.92fr)] lg:items-start">
      <div className="min-w-0"><div className="overflow-hidden border border-border bg-card"><img src={shot} alt={product.name} className="aspect-[4/5] w-full object-cover" /></div><div className="mt-4 flex max-w-full gap-3 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label={`Thư viện ${galleryCount} ảnh sản phẩm`}>{gallery.map((image, index) => <button key={`${image}-${index}`} type="button" onClick={() => setShot(image)} className={`size-20 shrink-0 overflow-hidden border transition-colors ${image === shot ? "border-primary" : "border-border hover:border-primary"}`} aria-label={`Chọn ảnh sản phẩm ${index + 1} trong ${galleryCount}`} aria-current={image === shot}><img src={image} alt="" className="size-full object-cover" loading="lazy" /></button>)}</div><p className="mt-2 text-[10px] uppercase tracking-[0.14em] text-silver">{galleryCount} {galleryCount === 1 ? "ảnh sản phẩm" : "ảnh sản phẩm"}</p></div>
    ...
    <section className="mt-16 border-t border-border pt-12"><div><p className="eyebrow">Reviews</p><h2 className="mt-3 text-2xl leading-tight sm:text-3xl">Đánh giá sản phẩm</h2></div><div className="mt-6 border border-border bg-card p-8 text-center"><p className="text-sm text-beige">Chưa có đánh giá cho sản phẩm này.</p><p className="mt-2 text-xs leading-5 text-silver">Hệ thống review sẽ chỉ hiển thị đánh giá thực từ khách hàng đã mua; WEARO không tạo dữ liệu đánh giá giả.</p></div></section>
