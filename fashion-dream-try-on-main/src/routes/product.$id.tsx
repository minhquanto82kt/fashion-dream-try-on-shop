...
function ProductPage() {
  ...
  async function addToCart() { if (!selectedVariant || outOfStock || loading) return; try { await add({ productId: product.id, size: selectedVariant.size, color: selectedVariant.color, qty: quantity }); toast.success(`Đã thêm ${product.name} (${selectedVariant.size} / ${selectedVariant.color}) vào giỏ`); } catch (error) { toast.error(error instanceof Error ? error.message : "Không thể thêm sản phẩm vào giỏ hàng."); } }
