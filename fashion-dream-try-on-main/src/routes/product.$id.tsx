  ...
  return (
    <div className="min-h-screen"><SiteNav />
      <main className="mx-auto max-w-7xl px-6 pb-24 pt-28 sm:px-12 lg:px-20">
        <div className="mb-8 flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-[0.16em] text-silver"><Link to="/shop" className="transition-colors hover:text-primary">Shop</Link><span>/</span><span>{product.category}</span><span>/</span><span className="max-w-[240px] truncate text-beige">{product.name}</span></div>
        <div className="grid min-w-0 gap-10 lg:grid-cols-[minmax(0,1.08fr)_minmax(360px,.92fr)] lg:items-start">
          <div className="min-w-0"><div className="overflow-hidden border border-border bg-card"><img src={shot} alt={product.name} className="aspect-[4/5] w-full object-cover" /></div><div className="mt-4 flex max-w-full gap-3 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{product.gallery.map((image, index) => <button key={`${image}-${index}`} type="button" onClick={() => setShot(image)} className={`size-20 shrink-0 overflow-hidden border transition-colors ${image === shot ? "border-primary" : "border-border hover:border-primary"}`} aria-label={`Chọn ảnh sản phẩm ${index + 1}`} aria-current={image === shot}><img src={image} alt="" className="size-full object-cover" loading="lazy" /></button>)}</div></div>
          <div className="min-w-0 lg:sticky lg:top-24"><div className="border-b border-border pb-7"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">{product.category}</p><h1 className="mt-3 max-w-2xl text-4xl leading-tight sm:text-5xl">{product.name}</h1></div>{product.badge && <span className="shrink-0 border border-primary px-2 py-1 text-[9px] uppercase tracking-[0.15em] text-primary">{product.badge}</span>}</div><p className="mt-5 font-display text-2xl leading-tight text-primary">{formatVnd(product.price)}</p><p className="mt-5 max-w-2xl text-sm leading-7 text-beige">{product.description}</p></div>
