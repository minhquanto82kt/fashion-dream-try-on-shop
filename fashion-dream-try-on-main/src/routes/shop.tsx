...
function ShopPage() {
  ...
  }, [products, cat, normalizedSearch, sort]);
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-7xl px-5 pb-24 pt-28 sm:px-10 lg:px-16">
        <section className="border-b border-border pb-8">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="eyebrow">Collection 2026 / Catalogue</p>
              <h1 className="mt-3 max-w-4xl text-5xl leading-[0.9] sm:text-6xl lg:text-7xl">
                {normalizedSearch ? (
                  <>
                    Kết quả <span className="text-primary">“{search}”</span>
                  </>
                ) : (
                  <>
                    {cat === "all" ? "Tất cả" : categoryName ?? "Danh mục"}{" "}
                    <span className="text-primary">sản phẩm</span>
                  </>
                )}
              </h1>
              <p className="mt-5 max-w-xl text-sm leading-6 text-silver">
                Chọn một item, xem chi tiết biến thể và chuyển thẳng sang AI TRY-ON khi bạn muốn thử phối đồ.
              </p>
            </div>

            <div className="w-full lg:max-w-sm">
              <label className="sr-only" htmlFor="shop-search">Tìm kiếm sản phẩm</label>
              <div className="flex items-center border border-border bg-background focus-within:border-primary">
                <Search size={16} className="ml-4 shrink-0 text-silver" aria-hidden="true" />
                <input
                  id="shop-search"
                  value={search}
                  onChange={(event) => {
                  ...
                  className="h-12 min-w-0 flex-1 bg-transparent px-3 text-sm text-foreground outline-none placeholder:text-silver"
                />
                {search && (
                  <button
                    type="button"
                    onClick={clearSearch}
                    className="mr-2 grid size-8 place-items-center text-silver hover:text-primary"
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3">
              <button
                ...
                className={`inline-flex items-center gap-2 border px-4 py-2 text-xs uppercase tracking-[0.15em] transition-colors ${
              ...
              <span className="text-xs uppercase tracking-[0.12em] text-silver">
                {list.length} sản phẩm
              </span>
            </div>

            <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
              <span className="text-[10px] uppercase tracking-[0.15em] text-silver">Sắp xếp</span>
              {SORTS.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  ...
                  className={`text-xs uppercase tracking-[0.12em] transition-colors ${
                    sort === item.id ? "text-primary" : "text-silver hover:text-beige"
                  }`}
                >
                  {item.label}
                </button>
          ...
          {filtersOpen && (
            <div className="mt-5 border-t border-border pt-5">
              <div className="flex flex-wrap gap-2">
                {[{ slug: "all", name: "Tất cả" }, ...CATEGORIES].map((category) => (
                  <button
                    key={category.slug}
                    type="button"
                    onClick={() => selectCategory(category.slug)}
                    className={`shrink-0 border px-4 py-2 text-xs uppercase tracking-[0.13em] transition-colors ${
                      cat === category.slug
              ...
              {activeFilterCount > 0 && (
                <button
                  ...
                  className="mt-4 text-[10px] uppercase tracking-[0.15em] text-primary hover:underline"
                >
                  Xóa toàn bộ bộ lọc
                </button>
              )}
        ...
        {list.length > 0 ? (
          <section className="mt-8" aria-label="Danh sách sản phẩm">
          ...
          <section className="mt-8 border-y border-border py-20 text-center">
            <p className="eyebrow">No match / 0 result</p>
            <h2 className="mt-3 text-3xl leading-tight sm:text-4xl">
              Không tìm thấy sản phẩm phù hợp.
            </h2>
            <p className="mx-auto mt-4 max-w-md text-sm leading-6 text-silver">
              Thử từ khóa khác hoặc xóa bộ lọc để xem lại toàn bộ catalogue.
            </p>
            <button
              type="button"
              onClick={clearFilters}
              className="mt-7 border border-primary px-5 py-3 text-xs uppercase tracking-[0.15em] text-primary transition-colors hover:bg-primary hover:text-primary-foreground"
            >
              Reset catalogue
            </button>
          </section>
