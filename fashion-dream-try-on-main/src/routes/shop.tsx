...
export const Route = createFileRoute("/shop")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Hoodie, tee, outerwear và phụ kiện WEARO. Chọn size, thử đồ ảo bằng AI rồi đặt hàng trong vài phút." },
      { property: "og:title", content: "Cửa hàng WEARO" },
      { property: "og:description", content: "Bộ sưu tập streetwear WEARO với AI virtual try-on." },
    ],
      ...
      <main className="mx-auto max-w-7xl px-5 pb-24 pt-28 sm:px-10 lg:px-16">
        <section className="border-b border-border pb-8">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div><p className="eyebrow">Collection 2026 / Catalogue</p><h1 className="mt-3 max-w-4xl text-5xl leading-[0.9] sm:text-6xl lg:text-7xl">{normalizedSearch ? <>Kết quả <span className="text-primary">“{search}”</span></> : <>{cat === "all" ? "Tất cả" : categoryName ?? "Danh mục"} <span className="text-primary">sản phẩm</span></>}</h1><p className="mt-5 max-w-xl text-sm leading-6 text-silver">Chọn một item, xem chi tiết biến thể và chuyển thẳng sang AI TRY-ON khi bạn muốn thử phối đồ.</p></div>
            <div className="w-full lg:max-w-sm"><label className="sr-only" htmlFor="shop-search">Tìm kiếm sản phẩm</label><div className="flex items-center border border-border bg-background focus-within:border-primary"><Search size={16} className="ml-4 shrink-0 text-silver" aria-hidden="true" /><input id="shop-search" value={search} onChange={(event) => { const value = event.target.value; setSearch(value); updateUrl({ search: value }); }} placeholder="Tìm sản phẩm..." className="h-12 min-w-0 flex-1 bg-transparent px-3 text-sm text-foreground outline-none placeholder:text-silver" />{search && <button type="button" onClick={clearSearch} className="mr-2 grid size-8 place-items-center text-silver hover:text-primary" aria-label="Xóa tìm kiếm"><X size={15} /></button>}</div></div>
          </div>
        </section>
        <section className="mt-6 border-b border-border pb-5">
          ...
          {filtersOpen && <div className="mt-5 border-t border-border pt-5"><div className="flex flex-wrap gap-2">{[{ slug: "all", name: "Tất cả" }, ...CATEGORIES].map((category) => <button key={category.slug} type="button" onClick={() => selectCategory(category.slug)} className={`shrink-0 border px-4 py-2 text-xs uppercase tracking-[0.13em] transition-colors ${cat === category.slug ? "border-primary bg-primary text-primary-foreground" : "border-border text-beige hover:border-primary"}`}>{category.name}</button>)}</div>{activeFilterCount > 0 && <button type="button" onClick={clearFilters} className="mt-4 text-[10px] uppercase tracking-[0.15em] text-primary hover:underline">Xóa toàn bộ bộ lọc</button>}</div>}
        </section>
