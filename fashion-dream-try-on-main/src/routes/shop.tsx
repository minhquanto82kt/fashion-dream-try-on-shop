...
function ShopPage() {
  ...
  return <div className="wearo-page wearo-shop-page min-h-screen bg-white text-[#171717]"><SiteNav /><main className="mx-auto max-w-7xl bg-white px-5 pb-24 pt-28 text-[#171717] sm:px-10 lg:px-16">
    <section className="border-b border-border pb-8"><div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between"><div><p className="eyebrow">Collection 2026 / Catalogue</p><h1 className="mt-3 max-w-4xl text-5xl leading-[0.9] sm:text-6xl lg:text-7xl">{normalizedSearch ? <>Kết quả <span className="text-primary">“{search}”</span></> : <>{cat === "all" ? "Tất cả" : categoryName ?? "Danh mục"} <span className="text-primary">sản phẩm</span></>}</h1><p className="mt-5 max-w-xl text-sm leading-6 text-silver">Modern everyday pieces cho nam và nữ — chọn một item, xem biến thể và chuyển thẳng sang AI TRY-ON khi bạn muốn thử phối đồ.</p></div><div className="w-full lg:max-w-sm"><label className="sr-only" htmlFor="shop-search">Tìm kiếm sản phẩm</label><div className="flex items-center border border-border bg-white focus-within:border-primary"><Search size={16} className="ml-4 shrink-0 text-silver" aria-hidden="true" /><input id="shop-search" value={search} onChange={(event) => { const value = event.target.value; setSearch(value); updateUrl({ search: value }); }} placeholder="Tìm sản phẩm..." className="h-12 min-w-0 flex-1 bg-transparent px-3 text-sm text-foreground outline-none placeholder:text-silver" />{search && <button type="button" onClick={clearSearch} className="mr-2 grid size-8 place-items-center text-silver hover:text-primary" aria-label="Xóa tìm kiếm"><X size={15} /></button>}</div></div></div></section>
