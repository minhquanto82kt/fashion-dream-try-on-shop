...
import wearoAiConsentCss from "../styles/wearo-ai-consent.css?url";
import wearoUiV2Css from "../styles/wearo-ui-v2.css?url";
import wearoUiV2FinalCss from "../styles/wearo-ui-v2-final.css?url";
...
function CustomerAuthGuard() { const router = useRouter(); const pathname = router.state.location.pathname; const [checking, setChecking] = useState(isCustomerProtectedPath(pathname)); useEffect(() => { let mounted = true; if (!isCustomerProtectedPath(pathname)) { setChecking(false); return () => { mounted = false; }; } setChecking(true); void getCustomerUser().then((user) => { if (!mounted) return; if (!user) { rememberReturnPath(); void router.navigate({ to: "/account" }); return; } setChecking(false); }); const onLogin = () => { if (!isCustomerProtectedPath(window.location.pathname)) return; void router.navigate({ to: consumeReturnPath() as never }); }; const onLogout = () => { if (isCustomerProtectedPath(window.location.pathname)) void router.navigate({ to: "/account" }); }; window.addEventListener("upthink:auth:login", onLogin); window.addEventListener("upthink:auth:logout", onLogout); return () => { mounted = false; window.removeEventListener("upthink:auth:login", onLogin); window.removeEventListener("upthink:auth:logout", onLogout); }; }, [pathname, router]); useEffect(() => startCustomerSessionWatcher(() => { if (isCustomerProtectedPath(window.location.pathname)) void getCustomerUser().then((user) => { if (!user) { rememberReturnPath(); void router.navigate({ to: "/account" }); } }); }), [router]); if (checking && isCustomerProtectedPath(pathname)) return <div className="min-h-screen bg-background text-foreground flex items-center justify-center"><p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Đang xác thực tài khoản…</p></div>; return null; }
...
function RootShell({ children }: { children: ReactNode }) { return <html lang="vi"><head><HeadContent /></head><body className="wearo-ui-v2">{children}<Scripts /></body></html>; }
