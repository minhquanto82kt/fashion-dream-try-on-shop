...
function isCustomerProtectedPath(pathname: string) {
  return CUSTOMER_PROTECTED_PREFIXES.some((prefix) => pathname === prefix || pathname.startsWith(`${prefix}/`));
}
...
function CustomerAuthGuard() {
  ...
  useEffect(() => {
    const onLogin = () => {
      ...
      if (returnTo !== "/" && isCustomerProtectedPath(new URL(returnTo, window.location.origin).pathname)) {
        void router.navigate({ to: returnTo as any });
      }
    };
  ...
  useEffect(() => {
    return startCustomerSessionWatcher(() => {
      if (isCustomerProtectedPath(window.location.pathname)) {
  ...
  if (checking && isCustomerProtectedPath(window.location.pathname)) {
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center">
        <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Đang xác thực tài khoản…</p>
      </div>
    );
  }
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Nền tảng thời trang của sinh viên IUH: streetwear, AI concept styling và virtual try-on." },
      { name: "author", content: "UpThink" },
      { property: "og:title", content: "UpThink" },
      { property: "og:description", content: "Streetwear cá nhân hóa với AI concept styling và virtual try-on." },
    ...
    links: [
      ...
      { rel: "stylesheet", href: uiStabilizationCss },
      { rel: "stylesheet", href: topbarCss },
      { rel: "stylesheet", href: adminUiCss },
      { rel: "stylesheet", href: adminComfortCss },
      { rel: "icon", href: "/favicon.ico", type: "image/x-icon" },
      { rel: "preconnect", href: "https://fonts.googleapis.com" },
      { rel: "preconnect", href: "https://fonts.gstatic.com" },
      { rel: "stylesheet", href: "https://fonts.googleapis.com/css2?family=Anton&family=Bodoni+Moda:opsz,wght@6..96,400..700&family=Oswald:wght@400;500;600;700&family=Space+Grotesk:wght@400;500;600&display=swap" },
    ],
  }),
