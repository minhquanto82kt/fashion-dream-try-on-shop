...
function AccountPage() {
  ...
  useEffect(() => {
    ...
    if (new URLSearchParams(window.location.search).get("verified") === "1") {
      setMessage("Email đã được xác nhận. Chào mừng bạn đến với WEARO.");
  ...
  if (checkingUser) return <main className="account-page"><SiteNav /><div className="account-loading">WEARO / VERIFYING ACCOUNT</div></main>;

  return (
    <div className="account-page">
      <style>{`
        .account-page{--account-cream:#f7f1e7;--account-orange:#f0a500;--account-ink:#1b1a17;position:relative;min-height:100vh;background:#1b1a17;color:var(--account-cream);isolation:isolate}
        .account-page :where(a,button,input){-webkit-tap-highlight-color:transparent}
        .account-content{position:relative;min-height:calc(100svh - 96px);overflow:hidden;background:#1b1a17}
        .account-content::before{content:"";position:absolute;inset:0;background-image:url('/account/account-background.png');background-size:cover;background-position:center top;background-repeat:no-repeat;z-index:0}
        .account-content::after{content:"";position:absolute;inset:0;background:linear-gradient(to bottom,rgba(27,26,23,0) 48%,rgba(27,26,23,.08) 58%,rgba(27,26,23,.48) 72%,#1b1a17 100%);z-index:1;pointer-events:none}
        .account-shell{position:relative;z-index:2;width:min(1180px,calc(100vw - 48px));min-height:calc(100svh - 96px);margin:auto;padding:30px 0 42px;display:flex;flex-direction:column}
        .account-main{flex:1;display:flex;align-items:center;padding:34px 0 12px}
        .account-loading{min-height:calc(100svh - 96px);display:grid;place-items:center;color:var(--account-orange);font:800 9px Arial,sans-serif;letter-spacing:.22em}
        .account-auth-grid{width:100%;display:grid;grid-template-columns:1fr 1fr;gap:70px;align-items:center}
        .account-kicker{color:var(--account-orange);font:800 10px/1.3 Arial,sans-serif;letter-spacing:.2em}
        .account-title{margin:12px 0 0;font-family:"Be Vietnam Pro","Space Grotesk",Arial,sans-serif;font-size:clamp(58px,8vw,112px);font-weight:900;line-height:.9;letter-spacing:-.055em;text-transform:uppercase}
        .account-title span{display:block}.account-title .accent{color:var(--account-orange)}
        .account-copy{max-width:430px;margin:28px 0 0;color:rgba(247,241,231,.78);font-size:13px;line-height:1.7}
        .account-index{margin-top:30px;color:rgba(247,241,231,.48);font:800 8px Arial,sans-serif;letter-spacing:.16em;text-transform:uppercase}.account-index b{color:var(--account-orange);margin-right:12px}
        .account-card{background:rgba(27,26,23,.86);border:1px solid rgba(247,241,231,.18);border-left:3px solid var(--account-orange);padding:32px;box-shadow:18px 18px 0 rgba(0,0,0,.22)}
        .account-card-head{display:flex;justify-content:space-between;gap:20px;margin-bottom:22px}.account-kicker-small{color:var(--account-orange);font:800 8px Arial,sans-serif;letter-spacing:.18em;text-transform:uppercase}.account-card h2{margin:7px 0 0;font:600 30px/1.1 Georgia,serif}.account-number{display:grid;place-items:center;width:32px;height:32px;border:1px solid rgba(240,165,0,.65);color:var(--account-orange);font:800 8px Arial,sans-serif}
        .account-switch{display:grid;grid-template-columns:1fr 1fr;border-top:1px solid rgba(247,241,231,.18);border-bottom:1px solid rgba(247,241,231,.18);margin-bottom:24px}.account-switch button{position:relative;padding:12px;border:0;background:transparent;color:rgba(247,241,231,.45);font:800 8px Arial,sans-serif;letter-spacing:.14em;text-transform:uppercase;cursor:pointer}.account-switch button[aria-selected=true]{color:var(--account-cream)}.account-switch button[aria-selected=true]:after{content:"";position:absolute;left:0;right:0;bottom:-1px;height:2px;background:var(--account-orange)}
        .account-google-wrap{display:flex;align-items:center;gap:10px;margin:24px 0 12px;color:rgba(247,241,231,.35);font:700 7px Arial,sans-serif;letter-spacing:.16em;text-transform:uppercase}.account-google-wrap:before,.account-google-wrap:after{content:"";height:1px;flex:1;background:rgba(247,241,231,.15)}
        .account-socials{display:grid;grid-template-columns:1fr 1fr;gap:10px}.account-social{width:100%;height:46px;display:flex;align-items:center;justify-content:center;gap:9px;border:1px solid rgba(247,241,231,.22);background:rgba(0,0,0,.16);color:var(--account-cream);font:800 8px Arial,sans-serif;letter-spacing:.1em;text-transform:uppercase;cursor:pointer}.account-social:hover{border-color:var(--account-orange);background:rgba(240,165,0,.06)}.account-social img{width:20px;height:20px;display:block;flex:none;object-fit:contain}
        .account-form{display:flex;flex-direction:column;gap:16px}.account-field{display:flex;flex-direction:column;gap:7px}.account-field label{font:800 8px Arial,sans-serif;letter-spacing:.14em;text-transform:uppercase;color:rgba(247,241,231,.68)}.account-field input{height:46px;width:100%;padding:0 12px;border:1px solid rgba(247,241,231,.18);background:rgba(0,0,0,.22);color:var(--account-cream);outline:none;border-radius:0}.account-field input:focus{border-color:var(--account-orange)}.account-password{position:relative}.account-password input{padding-right:46px}.account-eye{position:absolute;right:3px;top:3px;width:40px;height:40px;border:0;background:transparent;color:rgba(247,241,231,.55);cursor:pointer}.account-feedback{margin:0;font:9px/1.55 Arial,sans-serif}.account-feedback--error{color:#ffc5a8}.account-feedback--success{color:var(--account-cream)}.account-submit{min-height:48px;border:0;background:var(--account-orange);color:var(--account-ink);font:900 9px Arial,sans-serif;letter-spacing:.16em;text-transform:uppercase;cursor:pointer}.account-submit:disabled{opacity:.55;cursor:wait}.account-resend{border:0;background:transparent;color:var(--account-orange);font:800 8px Arial,sans-serif;letter-spacing:.12em;text-transform:uppercase;cursor:pointer;text-align:left}.account-resend:disabled{opacity:.5}
        .account-dashboard{width:100%}.account-welcome{display:flex;justify-content:space-between;align-items:flex-end;gap:30px;border-bottom:1px solid rgba(247,241,231,.18);padding-bottom:26px}.account-welcome h1{margin:8px 0 0;font-family:"Be Vietnam Pro","Space Grotesk",Arial,sans-serif;font-size:clamp(42px,6vw,78px);font-weight:900;line-height:.94;letter-spacing:-.045em;text-transform:uppercase}.account-welcome h1 span{color:var(--account-orange)}.account-welcome p{max-width:460px;color:rgba(247,241,231,.72);font-size:12px;line-height:1.7}.account-logout{display:inline-flex;align-items:center;gap:7px;margin-top:12px;border:1px solid rgba(247,241,231,.2);background:transparent;color:rgba(247,241,231,.78);padding:10px 13px;font:800 8px Arial,sans-serif;letter-spacing:.12em;text-transform:uppercase;cursor:pointer}.account-logout:hover{border-color:var(--account-orange);color:var(--account-orange)}
        .account-modules{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-top:18px}.account-module{min-height:150px;padding:20px;border:1px solid rgba(247,241,231,.15);background:rgba(27,26,23,.6);color:var(--account-cream);text-decoration:none;display:flex;flex-direction:column;justify-content:space-between;transition:.2s}.account-module:hover{transform:translateY(-3px);border-color:rgba(240,165,0,.55);background:rgba(240,165,0,.07)}.account-module-icon{color:var(--account-orange)}.account-module strong{font-size:13px}.account-module small{display:block;margin-top:6px;color:rgba(247,241,231,.52);font:800 8px Arial,sans-serif;letter-spacing:.12em;text-transform:uppercase}
        .account-profile{margin-top:18px;display:grid;grid-template-columns:1.3fr .7fr;gap:12px}.account-profile-card{padding:22px;border:1px solid rgba(247,241,231,.15);background:rgba(27,26,23,.66)}.account-profile-card h3{display:flex;align-items:center;margin:0 0 16px;color:var(--account-orange);font:800 12px/1.3 Arial,sans-serif;letter-spacing:.14em;text-transform:uppercase}.account-profile-row{display:flex;justify-content:space-between;gap:20px;padding:11px 0;border-bottom:1px solid rgba(247,241,231,.1);font-size:13px;line-height:1.5}.account-profile-row:last-child{border-bottom:0}.account-profile-row span{color:rgba(247,241,231,.5)}.account-profile-row strong{font-weight:600;text-align:right}.account-note{color:rgba(247,241,231,.72);font-size:14px;line-height:1.8}.account-note strong{color:var(--account-cream);font-weight:700}.account-bottom{padding-top:20px;color:rgba(247,241,231,.4);font:800 8px Arial,sans-serif;letter-spacing:.15em;text-transform:uppercase}.account-bottom b{color:var(--account-orange)}
        @media(max-width:900px){.account-shell{width:min(100% - 40px,720px)}.account-auth-grid{grid-template-columns:1fr;gap:30px}.account-modules{grid-template-columns:repeat(2,1fr)}.account-profile{grid-template-columns:1fr}.account-welcome{align-items:flex-start;flex-direction:column}.account-title{font-size:clamp(54px,15vw,84px)}}
        @media(max-width:560px){.account-content{min-height:calc(100svh - 72px)}.account-shell{width:calc(100vw - 28px);min-height:calc(100svh - 72px);padding:24px 0 30px}.account-main{padding:30px 0}.account-card{padding:22px 18px}.account-modules{grid-template-columns:1fr}.account-module{min-height:125px}.account-social{font-size:7px;letter-spacing:.06em}.account-socials{gap:8px}.account-profile-row{align-items:flex-start;flex-direction:column;gap:5px}.account-profile-row strong{text-align:left}.account-welcome h1{font-size:clamp(38px,11vw,60px)}}
      `}</style>
      ...
      <main className="account-content">
        <div className="account-shell">
            ...
            {user ? (
              <div className="account-dashboard">
                <div className="account-welcome">
                  <div><div className="account-kicker">WEARO / PERSONAL FASHION SYSTEM</div><h1>CHÀO MỪNG <span>{user.user_metadata?.full_name || user.user_metadata?.name || user.email?.split("@")[0] || "BẠN"}</span></h1></div>
              ...
              <div className="account-auth-grid">
                ...
                <div className="account-card">
                  ...
                  <div className="account-socials">
                    <button className="account-social" type="button" onClick={signInWithGoogle} disabled={loading}><img src="/brand/google-g.svg" alt="Google"/><span>Google</span></button>
          ...
          <div className="account-bottom"><b>●</b> SYSTEM ONLINE / CUSTOMER ACCOUNT / WEARO 2026</div>
        </div>
      </main>
