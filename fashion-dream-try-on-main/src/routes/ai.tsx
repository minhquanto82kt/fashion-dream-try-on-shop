    ...
    <div className="border-t border-border p-4 sm:p-6">
      ...
      <div className="mt-5 border-t border-border pt-4"><div className="mb-2 flex items-center justify-between"><label htmlFor="ai-stylist-brief" className="text-[10px] font-medium uppercase tracking-[0.18em] text-silver">04 · Your brief</label><span className={`text-[10px] ${prompt.length >= 540 ? "text-primary" : "text-silver"}`}>{prompt.length}/600</span></div>
        <div className="relative rounded-2xl border border-border bg-background p-2 focus-within:border-primary">
          <textarea id="ai-stylist-brief" ref={textRef} rows={4} maxLength={600} value={prompt} onChange={(e) => onPromptChange(e.target.value)} placeholder="Mô tả outfit bạn muốn… Ví dụ: tông đen trắng, form rộng, phối @Shadow Hoodie với sneaker trắng." className="w-full resize-none bg-transparent px-3 py-2 text-sm leading-6 text-foreground outline-none placeholder:text-silver" />
...
function TryOnWorkspace({ initialProduct }: { initialProduct?: string }) {
  ...
  const productsQuery = useQuery({ queryKey: ["ai-products"], queryFn: () => listProducts(), staleTime: 60_000 }); const products = (productsQuery.data ?? []) as AiProduct[]; const product = products.find((p) => p.id === productId) ?? products[0];
  ...
  const onFileChange = (file: File | undefined) => { if (!file) return; const allowed = ["image/jpeg", "image/png", "image/webp"]; if (!allowed.includes(file.type)) { toast.error("Chỉ hỗ trợ JPG, PNG hoặc WEBP."); return; } if (file.size > 6 * 1024 * 1024) { toast.error("Ảnh tối đa 6MB."); return; } const reader = new FileReader(); reader.onload = () => setPerson(typeof reader.result === "string" ? reader.result : null); reader.onerror = () => toast.error("Không thể đọc ảnh. Vui lòng thử lại."); reader.readAsDataURL(file); };
  ...
  if (productsQuery.isError || !product) return <><ResultFrame pending={false} emptyLabel="Chưa có sản phẩm phù hợp để thử đồ." /><div className="border-t border-border p-6 text-sm text-silver">Không thể tải sản phẩm thử đồ hoặc hiện chưa có sản phẩm đã xuất bản có hình ảnh và biến thể.</div></>;
  const resultReady = Boolean(mutation.data?.image); const addLookToCart = () => { add({ productId: product.id, size: product.defaultVariant.size, color: product.defaultVariant.color, qty: 1 }); toast.success("Đã thêm look vào giỏ hàng."); };
  return <>
    ...
    <div className="border-t border-border p-4 sm:p-6">
      <div className="mb-5"><div className="mb-2 flex items-center justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[0.18em] text-primary">Bước 1 · Ảnh của bạn</p><p className="mt-1 text-xs text-silver">Ảnh toàn thân giúp AI nhận diện form và tỷ lệ tốt hơn.</p></div>{person && <span className="text-[10px] uppercase tracking-[0.14em] text-primary">✓ Đã tải ảnh</span>}</div><button type="button" onClick={() => fileRef.current?.click()} className="flex min-h-28 w-full items-center justify-center gap-3 rounded-2xl border border-dashed border-border bg-background px-5 py-5 text-left text-sm text-beige transition-colors hover:border-primary hover:bg-primary/5"><Upload className="size-5 shrink-0 text-primary" /><span><strong className="block text-sm text-foreground">{person ? "Đổi ảnh toàn thân" : "TẢI ẢNH TOÀN THÂN"}</strong><span className="mt-1 block text-xs text-silver">JPG, PNG hoặc WebP · tối đa 6MB</span></span></button><input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(e) => { onFileChange(e.target.files?.[0]); e.currentTarget.value = ""; }} />{person && <div className="mt-3 flex items-center gap-3 rounded-xl border border-border bg-background p-2"><img src={person} alt="Ảnh của bạn" className="size-20 rounded-lg object-cover" /><div className="min-w-0"><p className="text-sm font-medium text-foreground">Ảnh sẵn sàng</p><p className="mt-1 text-xs text-silver">Bạn có thể chọn sản phẩm bên dưới.</p></div></div>}</div>
      <div className="mb-5"><div className="mb-2 flex items-end justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[0.18em] text-primary">Bước 2 · Chọn sản phẩm</p><p className="mt-1 text-xs text-silver">Chọn một sản phẩm để thử.</p></div><span className="text-xs text-silver">{products.length} sản phẩm</span></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{products.map((p) => <button key={p.id} type="button" onClick={() => setProductId(p.id)} aria-pressed={p.id === product.id} className={`group overflow-hidden rounded-xl border text-left transition-all ${p.id === product.id ? "border-primary ring-1 ring-primary" : "border-border hover:border-primary/60"}`}><div className="relative aspect-[4/5] overflow-hidden bg-background"><img src={p.image} alt={p.name} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-[1.03]" />{p.id === product.id && <span className="absolute right-2 top-2 rounded-full bg-primary px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-primary-foreground">Đang chọn</span>}</div><div className="p-3"><p className="truncate text-xs font-medium text-foreground">{p.name}</p><p className="mt-1 text-xs text-primary">{formatVnd(p.price)}</p><p className="mt-1 text-[10px] uppercase tracking-[0.12em] text-silver">{p.category}</p></div></button>)}</div></div>
