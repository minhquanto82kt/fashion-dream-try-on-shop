...
function AdminAppearancePage() {
  ...
  return (
    <div className="up-theme-page">
      <style>{`/* P0-A appearance control center styles retained; palette CRUD styles included here. */
      ...
      <header className="up-theme-header">
        <div><div className="up-theme-kicker">ADMIN / APPEARANCE / BRANDING</div><h1>Appearance</h1><p>Control brand assets, colors and the visual system from one workspace.</p>{message && <div className="up-theme-feedback success">{message}</div>}{error && <div className="up-theme-feedback error">{error}</div>}</div>
      ...
      <div className="up-theme-layout">
        <main className="up-theme-main">
          ...
          <section id="appearance-colors" className="up-theme-panel"><h2>Global palette</h2><p>Edit the six global theme tokens. Changes are previewed immediately and only become site-wide after Save Theme.</p><div className="up-theme-color-grid">{COLOR_FIELDS.map(({ key, label, description }) => <div className="up-theme-color" key={key}><input aria-label={`${label} color picker`} type="color" value={isValidHexColor(colors[key]) ? colors[key] : "#000000"} onChange={(event) => updateColor(key, event.target.value.toUpperCase())} /><div><strong>{label}</strong><small>{description}</small></div><input aria-label={`${label} HEX`} type="text" value={colors[key]} maxLength={7} onChange={(event) => updateColor(key, event.target.value.toUpperCase())} /></div>)}</div></section>

          <section id="appearance-palette-library" className="up-theme-panel"><div className="up-theme-palette-header"><div className="up-theme-palette-heading"><h2>Palette Library</h2><button type="button" className={`up-theme-more${paletteLibraryOpen ? " active" : ""}`} aria-label={paletteLibraryOpen ? "Collapse palette library" : "View more palettes"} title={paletteLibraryOpen ? "Thu gọn" : "Xem thêm"} onClick={() => setPaletteLibraryOpen((open) => !open)}>{paletteLibraryOpen ? <ChevronUp size={15} /> : <MoreHorizontal size={15} />}</button></div><button type="button" className="up-theme-palette-add" onClick={startCreatePalette}><Plus size={12} /> New palette</button></div><p>Save reusable color systems with a name. Select a palette to read its HEX values, edit its colors, update it, or delete it.</p><div className="up-theme-palette-tools"><div className="up-theme-palette-search"><Search size={13} /><input value={paletteSearch} onChange={(event) => setPaletteSearch(event.target.value)} placeholder="Search palette name…" aria-label="Search palettes" /></div></div>{(paletteLibraryOpen || editingPaletteId) && <div className="up-theme-palette-editor"><div><label htmlFor="palette-name">{editingPaletteId ? "Update palette" : "Create palette"}</label><input id="palette-name" value={paletteName} maxLength={80} onChange={(event) => setPaletteName(event.target.value)} placeholder="e.g. WEARO Autumn 2026" /></div><div className="up-theme-palette-editor-actions"><button type="button" className="up-theme-palette-save" disabled={paletteBusy} onClick={() => void savePalette()}>{editingPaletteId ? <Pencil size={11} /> : <Check size={11} />}{paletteBusy ? "Saving…" : editingPaletteId ? "Update" : "Save"}</button>{editingPaletteId && <button type="button" className="up-theme-palette-cancel" onClick={cancelPaletteEdit}><X size={11} />Cancel</button>}</div></div>}<div className="up-theme-palette-list">{visiblePalettes.length === 0 ? <div className="up-theme-palette-empty">{paletteSearch ? "No palettes match your search." : "No saved palettes yet. Create one from the current colors."}</div> : visiblePalettes.map((palette) => <div className="up-theme-palette-card" key={palette.id}><div className="up-theme-palette-card-main" onClick={() => applyPalette(palette, palette.name)} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") applyPalette(palette, palette.name); }}><div className="up-theme-palette-name">{palette.name}<span className="up-theme-palette-count">6 TOKENS</span></div><div className="up-theme-palette-swatches">{COLOR_FIELDS.map(({ key, label }) => <div className="up-theme-palette-swatch" key={key} title={`${label}: ${palette[key]}`}><i style={{ background: palette[key] }} /><code>{palette[key]}</code></div>)}</div><div className="up-theme-palette-meta">Updated {new Date(palette.updated_at).toLocaleString("vi-VN")}</div></div><div className="up-theme-palette-actions"><button type="button" className="up-theme-icon-button" title="Edit palette" aria-label={`Edit ${palette.name}`} onClick={() => startEditPalette(palette)}><Pencil size={13} /></button><button type="button" className="up-theme-icon-button" title="Copy palette name" aria-label={`Copy ${palette.name}`} onClick={() => { copyText(palette.name); setMessage(`Copied palette name “${palette.name}”.`); }}><Copy size={13} /></button><button type="button" className="up-theme-icon-button danger" title="Delete palette" aria-label={`Delete ${palette.name}`} disabled={paletteBusy} onClick={() => void removePalette(palette)}><Trash2 size={13} /></button></div></div>)}</div>{!paletteLibraryOpen && filteredPalettes.length > 4 && <button type="button" className="up-theme-palette-add" style={{ marginTop: 9, width: "100%", justifyContent: "center" }} onClick={() => setPaletteLibraryOpen(true)}><MoreHorizontal size={12} /> View {filteredPalettes.length - 4} more palettes</button>}</section>
