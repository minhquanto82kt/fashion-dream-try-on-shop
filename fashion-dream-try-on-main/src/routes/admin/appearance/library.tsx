...
function AppearanceThemeLibraryPage() {
  ...
  const differences = useMemo(() => selected && compare && selected.id !== compare.id ? diffKeys(selected, compare) : [], [selected, compare]);
  ...
  return <div className="up-theme-library">
    ...
    `}</style>
    <header className="up-theme-library-header"><div><div className="up-theme-library-kicker">APPEARANCE / LIBRARY</div><h1>Theme Library</h1><p>Saved themes act as immutable history. Compare snapshots, load an older theme as a draft, then publish deliberately.</p></div><Link to="/admin/appearance">← Appearance</Link></header>
    ...
    <div className="up-theme-library-layout">
      <section className="up-theme-library-list">{loading ? <div className="up-theme-library-empty">Loading theme history…</div> : themes.length === 0 ? <div className="up-theme-library-empty">No saved themes yet.</div> : themes.map((theme) => <button key={theme.id} type="button" className={`up-theme-library-row${selected?.id === theme.id ? " active" : ""}`} onClick={() => selectTheme(theme)}><span><span className="up-theme-library-name">{theme.name}</span><span className="up-theme-library-meta">{formatDate(theme.updated_at)}{theme.published_at ? ` · Published ${formatDate(theme.published_at)}` : ""}</span></span><span className={`up-theme-library-status ${theme.status}`}>{theme.status}</span></button>)}</section>
      <aside className="up-theme-library-detail">{selected ? <><h2>{selected.name}</h2><small>{selected.id}</small><div className="up-theme-library-swatches">{Object.entries(selected.theme_data).map(([key,value]) => <span key={key} title={`${key}: ${value}`} style={{background:value}} />)}</div><dl>{Object.entries(selected.theme_data).map(([key,value]) => <div key={key}><dt>{key}</dt><dd>{value}</dd></div>)}</dl><div className="up-theme-library-actions"><button type="button" onClick={() => void openThemePreview(selected.theme_data)} disabled={busy}><Eye size={13}/> Preview</button><button type="button" onClick={() => { setCompare(null); setCompareMode((value) => !value); setMessage(compareMode ? "Compare mode cancelled." : "Compare mode: select another theme from the list."); }} disabled={busy}><GitCompare size={13}/> {compareMode ? "Cancel compare" : "Compare"}</button>{selected.status !== "draft" && <button type="button" onClick={() => void rollback(selected)} disabled={busy}><RotateCcw size={13}/> Load as draft</button>}{selected.status === "draft" && <button type="button" className="accent" onClick={() => void publish(selected)} disabled={busy}><Send size={13}/> Publish</button>}{selected.status === "draft" && <button type="button" onClick={() => void remove(selected)} disabled={busy}><Trash2 size={13}/> Delete</button>}<button type="button" onClick={() => { void navigator.clipboard?.writeText(JSON.stringify(selected.theme_data,null,2)); setMessage("Theme JSON copied."); }} disabled={busy}><Copy size={13}/> Copy JSON</button></div>{compareMode && <div className="up-theme-library-compare">Compare mode is active. Select a different theme from the list to compare it with “{selected.name}”.</div>}{compare && compare.id !== selected.id && <div className="up-theme-library-compare"><strong>Compare with {compare.name}</strong><br/>{differences.length ? `${differences.length} token(s) differ: ${differences.join(", ")}.` : "No color token differences."}</div>}</>}</aside>
    </div>
