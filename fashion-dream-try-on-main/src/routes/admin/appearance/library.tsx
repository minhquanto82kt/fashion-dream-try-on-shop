...
function AppearanceThemeLibraryPage() {
  ...
  return <div className="up-theme-library">
    ...
    <div className="up-theme-library-layout">
      ...
      <aside className="up-theme-library-detail">{selected ? <><h2>{selected.name}</h2><small>{selected.id}</small><div className="up-theme-library-swatches">{Object.entries(selected.theme_data).map(([key,value]) => <span key={key} title={`${key}: ${value}`} style={{background:value}} />)}</div><dl>{Object.entries(selected.theme_data).map(([key,value]) => <div key={key}><dt>{key}</dt><dd>{value}</dd></div>)}</dl><div className="up-theme-library-actions"><button type="button" onClick={() => void openThemePreview(selected.theme_data)} disabled={busy}><Eye size={13}/> Preview</button><button type="button" onClick={() => setCompare(compare?.id === selected.id ? null : selected)} disabled={busy}><GitCompare size={13}/> Compare</button>{selected.status !== "draft" && <button type="button" onClick={() => void rollback(selected)} disabled={busy}><RotateCcw size={13}/> Load as draft</button>}{selected.status === "draft" && <button type="button" className="accent" onClick={() => void publish(selected)} disabled={busy}><Send size={13}/> Publish</button>}{selected.status === "draft" && <button type="button" onClick={() => void remove(selected)} disabled={busy}><Trash2 size={13}/> Delete</button>}<button type="button" onClick={() => { void navigator.clipboard?.writeText(JSON.stringify(selected.theme_data,null,2)); setMessage("Theme JSON copied."); }} disabled={busy}><Copy size={13}/> Copy JSON</button></div>{compare && compare.id !== selected.id && <div className="up-theme-library-compare"><strong>Compare with {compare.name}</strong><br/>{differences.length ? `${differences.length} token(s) differ: ${differences.join(", ")}.` : "No color token differences."}</div>}</> : <div className="up-theme-library-empty">Select a saved theme.</div>}</aside>
    </div>
