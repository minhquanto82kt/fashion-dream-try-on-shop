...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setHydrated(false);
      if (mockMode || !authenticated) { setLines(mockMode ? readLocalCart(MOCK_STORAGE_KEY) : readGuestCart()); setHydrated(true); return; }
      ...
      try {
        ...
        if (guestLines.length > 0) {
          ...
          for (const line of guestLines) { const variant = variantByKey.get(`${line.productId}|${line.size}|${line.color}`); if (!variant) continue; try { serverCart = await addServerCartItem({ data: { accessToken: session.access_token, variantId: variant.id, quantity: line.qty } }); } catch (error) { console.warn("Không thể nhập một dòng guest cart vào tài khoản:", error); } }
          localStorage.removeItem(GUEST_STORAGE_KEY); localStorage.removeItem(LEGACY_GUEST_STORAGE_KEY);
        }
        if (!cancelled) setLines(serverCartToLines(serverCart));
      } catch (error) { console.error("Không thể tải giỏ hàng tài khoản:", error); if (!cancelled) setLines([]); }
  ...
  function add(line: CartLine) {
    if (mockMode || !authenticated) { setLines((current) => { const index = current.findIndex((item) => item.productId === line.productId && item.size === line.size && item.color === line.color); if (index === -1) return [...current, { ...line, qty: clampQty(line.qty) }]; return current.map((item, itemIndex) => itemIndex === index ? { ...item, qty: clampQty(item.qty + line.qty) } : item); }); return; }
    const session = getCustomerSession(); const product = products.find((item) => item.id === line.productId); const variant = product?.variants.find((item) => item.size === line.size && item.color === line.color); if (!session?.access_token || !variant) return;
    setLoading(true); void addServerCartItem({ data: { accessToken: session.access_token, variantId: variant.id, quantity: clampQty(line.qty) } }).then((cart) => setLines(serverCartToLines(cart))).catch((error) => alert(error instanceof Error ? error.message : "Không thể thêm sản phẩm vào giỏ hàng.")).finally(() => setLoading(false));
  }
