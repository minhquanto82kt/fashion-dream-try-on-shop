...
function createOrderCode() {
  ...
  return `FD-${date}-${Math.random().toString(36).slice(2, 7).toUpperCase()}`;
}
...
async function resolveCustomerFromToken(accessToken: string): Promise<AuthUser> {
  ...
  const response = await fetchWithTimeoutAndRetry(`${url}/auth/v1/user`, {
...
export async function getCustomerOrderByToken(accessToken: string, orderIdOrCode: string): Promise<CustomerOrderDetail | null> {
  ...
  if (!key) return null;
  const select = "id,order_code,customer_name,phone,email,address,city,district,payment_method,payment_status,order_status,subtotal,shipping_fee,total,note,created_at,user_id";
  let orders = await supabaseRequest<CustomerOrder[]>(`orders?id=eq.${encodeURIComponent(key)}&select=${select}&limit=1`);
  if (!orders.length) orders = await supabaseRequest<CustomerOrder[]>(`orders?order_code=eq.${encodeURIComponent(key)}&select=${select}&limit=1`);
  const order = orders[0];
  if (!order || order.user_id !== user.id) return null;
  const items = await supabaseRequest<CustomerOrderItem[]>(`order_items?order_id=eq.${encodeURIComponent(order.id)}&select=*&order=created_at.asc`);
  return { ...order, items };
}
...
export const createOrder = createServerFn({ method: "POST" }).validator((data: CreateOrderInput) => data).handler(async ({ data }) => {
  ...
  if (data.mockUser) {
    const orderCode = `MOCK-${new Date().toISOString().slice(0, 10).replaceAll("-", "")}-${Math.random().toString(36).slice(2, 7).toUpperCase()}`;
    return { orderId: `mock-order-${Date.now()}`, orderCode, total: 0, mock: true, itemCount: data.items.reduce((s, i) => s + i.quantity, 0) };
  }
