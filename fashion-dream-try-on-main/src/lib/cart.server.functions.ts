...
async function getCartForUser(userId: string) {
  ...
  const items = await supabaseRequest<CartItemRow[]>(
  ...
  const variants = await supabaseRequest<VariantRow[]>(
...
export const addServerCartItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const existing = await supabaseRequest<CartItemRow[]>(
    ...
    if (existing[0]) {
      await supabaseRequest(`cart_items?id=eq.${encodeURIComponent(existing[0].id)}&cart_id=eq.${encodeURIComponent(cart.id)}`, {
        method: "PATCH",
...
export const updateServerCartItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await supabaseRequest(`cart_items?id=eq.${encodeURIComponent(data.itemId)}&cart_id=eq.${encodeURIComponent(cart.id)}`, {
      method: "PATCH",
