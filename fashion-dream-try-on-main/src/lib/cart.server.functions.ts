...
// Cart mutations run on the server only. The user's identity is derived from
// the Supabase access token first, then the server secret key is used for the
// actual cart CRUD so RLS on cart tables cannot make a successful mutation
// disappear from the response. Ownership is still enforced by user_id/cart_id.
...
export const getServerCart = createServerFn({ method: "POST" }).validator((data: { accessToken: string }) => data).handler(async ({ data }) => {
...
export const addServerCartItem = createServerFn({ method: "POST" }).validator((data: { accessToken: string; variantId: string; quantity: number }) => data).handler(async ({ data }) => {
  ...
  if (existing[0]) {
    await supabaseRequest(`cart_items?id=eq.${encodeURIComponent(existing[0].id)}&cart_id=eq.${encodeURIComponent(cart.id)}`, { method: "PATCH", body: JSON.stringify({ quantity: nextQuantity, updated_at: new Date().toISOString() }) });
  } else {
    await supabaseRequest("cart_items", { method: "POST", body: JSON.stringify({ cart_id: cart.id, variant_id: data.variantId, quantity: data.quantity }) });
  }
  return getCartForUser(user.id);
});
