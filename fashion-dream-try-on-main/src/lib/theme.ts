...
export function sanitizeThemeColors(value: unknown): ThemeColors {
  ...
  for (const key of Object.keys(DEFAULT_THEME_COLORS) as Array<keyof ThemeColors>) {
    if (typeof candidate[key] === "string" && isValidHexColor(candidate[key])) {
      next[key] = candidate[key] as string;
    }
  }
...
export function resetTheme(): ThemeColors {
  ...
  if (typeof window !== "undefined") {
    window.dispatchEvent(new CustomEvent("upthink:theme:changed", { detail: DEFAULT_THEME_COLORS }));
  }
  return DEFAULT_THEME_COLORS;
}
