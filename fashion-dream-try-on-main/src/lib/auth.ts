...
function syncCartForCustomer(userId: string) {
  ...
  const customerCart = readCart(customerKey);
  const merged = mergeCartLines(customerCart, guestCart);
...
function restoreCustomerCart(userId: string) {
  if (typeof window === "undefined") return;
  const customerKey = `${CUSTOMER_CART_PREFIX}${userId}`;
  window.localStorage.setItem(GUEST_CART_KEY, JSON.stringify(readCart(customerKey)));
}
...
async function parseAuthResponse(response: Response): Promise<AuthResponse> {
  ...
  if (!response.ok) {
    throw new Error(data.msg || data.message || data.error_description || "Yêu cầu xác thực thất bại.");
  }
  return data;
}
...
export async function signInCustomer(email: string, password: string) {
  ...
  if (data.access_token && (await checkAdminWithCustomerToken(data.access_token))) {
    return rejectAdminCustomerSession(data.access_token, "Tài khoản này dành cho quản trị viên. Hãy đăng nhập bằng tài khoản mua hàng riêng.");
  }
  setCustomerSession(data);
  if (data.user?.id) restoreCustomerCart(data.user.id);
...
export async function signUpCustomer(email: string, password: string) {
  ...
  if (data.access_token) {
    if (await checkAdminWithCustomerToken(data.access_token)) {
      return rejectAdminCustomerSession(data.access_token, "Email này thuộc tài khoản quản trị viên và không thể đăng ký tài khoản mua hàng.");
    }
    setCustomerSession(data);
...
export async function getCustomerUser() {
  ...
  let response = await fetch(`${supabaseConfig.url}/auth/v1/user`, {
    headers: { apikey: supabaseConfig.key, Authorization: `Bearer ${session.access_token}` },
  });
  if (response.status === 401) {
    const refreshed = await refreshCustomerSession();
    if (!refreshed?.access_token) return null;
    response = await fetch(`${supabaseConfig.url}/auth/v1/user`, {
      headers: { apikey: supabaseConfig.key, Authorization: `Bearer ${refreshed.access_token}` },
    });
  }
  if (!response.ok) { setCustomerSession(null); return null; }
  try {
    if (await checkAdminWithCustomerToken(getCustomerSession()!.access_token)) {
      setCustomerSession(null); return null;
    }
  } catch {
    setCustomerSession(null); return null;
  }
