...
async function requireAdmin(accessToken: string) {
  ...
  const admins = await supabaseRequest<{ user_id: string }[]>(
...
function handleAuthError(error: unknown): never {
  if (error instanceof Error && error.message === "UNAUTHORIZED") {
    throw new Error("Phiên quản trị không hợp lệ hoặc đã hết hạn.");
  }
  if (error instanceof Error && error.message === "FORBIDDEN") {
    throw new Error("Bạn không có quyền quản trị đơn hàng.");
  }
  throw error;
}
...
export const listAdminOrders = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      return await supabaseRequest<Order[]>(
...
export const getAdminOrder = createServerFn({ method: "POST" })
  .validator((data: AdminInput & { orderId: string }) => data)
  .handler(async ({ data }) => {
    try {
      await requireAdmin(data.accessToken);
...
export const updateAdminOrderStatus = createServerFn({ method: "POST" })
  .validator((data: AdminInput & { orderId: string; orderStatus: string }) => data)
  .handler(async ({ data }) => {
    try {
      await requireAdmin(data.accessToken);
      ...
      const rows = await supabaseRequest<Order[]>(
        `orders?id=eq.${encodeURIComponent(data.orderId.trim())}`,
        {
          method: "PATCH",
          body: JSON.stringify({ order_status: data.orderStatus }),
        },
      );
