...
export const generateConcept = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const prompt = [
      ...
      `Style: ${data.style}.`, `Occasion: ${data.occasion}.`,
      ...
      "Natural daylight.", "35mm photography.", "Sharp realistic detail.", "Photorealistic.", "No text.", "No watermark.",
    ].filter(Boolean).join(" ");
...
export const generateTryOn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const prompt = [
      ...
      "Preserve the person's identity and facial features.", "Preserve body proportions, skin tone, pose and hairstyle.",
      "Keep the original background and camera composition.", "Replace only the clothing.",
      ...
      "Do not change the person's face or body.", data.note ? `Additional request: ${data.note}.` : "",
      "Photorealistic result.", "No text.", "No watermark.",
    ].filter(Boolean).join(" ");
