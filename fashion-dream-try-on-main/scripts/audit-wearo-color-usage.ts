...
const COLOR_ALIASES: Record<string, ColorName> = {
  ...
  'var(--wearo-white)': 'white',
  'var(--wearo-black)': 'black',
  'var(--white)': 'white',
  'var(--black)': 'black',
};
...
function canonicalColor(value: string): ColorName | null {
  return COLOR_ALIASES[normalize(value)] ?? null;
}
...
function auditCssFile(path: string): string[] {
  ...
  while ((match = blockPattern.exec(source))) {
    ...
    const body = match[2];
    const declarations = [...body.matchAll(/(?:^|[;\s])(color|background-color|background)\s*:\s*([^;]+)/gi)]
    ...
    for (const text of textColors) {
      ...
      for (const background of backgrounds) {
        ...
        if (textColor === backgroundColor || ratio < 2) {
          failures.push(`${relative(process.cwd(), path)} :: ${selector} :: ${text.property}=${text.value} vs ${background.property}=${background.value} :: contrast=${ratio.toFixed(2)} :: RESTRICTED`);
        }
      }
    }
  }
...
function auditMarkupFile(path: string): string[] {
  ...
  for (const match of source.matchAll(/className=(?:"([^"]+)"|'([^']+)'|\{`([^`]+)`\})/g)) {
    ...
    for (const text of textMatches) {
      const textColor = canonicalColor(`#${text[1].length === 3 ? text[1].split('').map((c) => c + c).join('') : text[1]}`);
      if (!textColor) continue;
      for (const background of backgroundMatches) {
        const backgroundColor = canonicalColor(`#${background[1].length === 3 ? background[1].split('').map((c) => c + c).join('') : background[1]}`);
        if (!backgroundColor) continue;
        const ratio = contrast(textColor, backgroundColor);
        if (textColor === backgroundColor || ratio < 2) {
          failures.push(`${relative(process.cwd(), path)} :: className="${className}" :: text=${text[0]} vs background=${background[0]} :: contrast=${ratio.toFixed(2)} :: RESTRICTED`);
        }
      }
    }
  }
...
const markupFiles = files.filter((path) => /\.(tsx|ts)$/.test(path));
const failures = [
  ...cssFiles.flatMap(auditCssFile),
  ...markupFiles.flatMap(auditMarkupFile),
];

console.log('WEARO COLOR USAGE AUDIT');
console.log('=======================');
console.log(`CSS files scanned: ${cssFiles.length}`);
console.log(`TS/TSX files scanned: ${markupFiles.length}`);
console.log(`Restricted text/background collisions: ${failures.length}`);
