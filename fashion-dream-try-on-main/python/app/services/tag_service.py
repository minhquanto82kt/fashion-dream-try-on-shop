...
def get_tag(tag_id: str) -> dict[str, Any]:
    ...
    response = (
        get_supabase_client().table("tags").select("*").eq("id", tag_id).limit(1).execute()
    )
    if not response.data:
        raise TagNotFoundError(tag_id)
    row = response.data[0]
    links = (
        get_supabase_client().table("product_tags").select("tag_id").eq("tag_id", tag_id).execute()
    )
...
def delete_tag(tag_id: str) -> None:
    """Delete a tag. Product links must be removable by the database relationship."""
    get_tag(tag_id)
