...
function getVercelConfig() {
  ...
  if (!token) {
    throw new Error("Thiếu VERCEL_ACCESS_TOKEN trên Vercel.");
  }
...
async function getFlag(token: string) {
  const response = await fetch(
    `https://api.vercel.com/v1/projects/${PROJECT_ID}/feature-flags/flags/${FLAG_SLUG}?slug=${encodeURIComponent(TEAM_SLUG)}`,
    {
      headers: {
...
async function updateFlag(
  ...
  const response = await fetch(
    `https://api.vercel.com/v1/projects/${PROJECT_ID}/feature-flags/flags/${FLAG_SLUG}?slug=${encodeURIComponent(TEAM_SLUG)}`,
    {
      ...
      body: JSON.stringify({
        message: `Admin toggle ai_try_on ${environment}: ${enabled ? "ON" : "OFF"}`,
        environments: {
    ...
    handlers: {
      ...
      PATCH: async ({ request }) => {
        try {
          ...
          const { token } = getVercelConfig();
          const result = await updateFlag(token, environment, enabled);
